...
```{r}
shorezone_gis <- st_read("zoning.gdb","shorezone_tolerance") %>%
...
```{r}
area.plan<-read_excel("data/Area Plan Use Table.xlsx", sheet="Sheet3", skip=2)
...
area_gis <- st_read("zoning.gdb","area_plans_gis")
tvap_os <- st_read("zoning.gdb","tvap_os") %>%
  mutate(id="Open Space-TC-OS", Local_Plan_Name="TAHOE VALLEY AREA PLAN") %>%
  select(id, Local_Plan_Name)
special <- st_read("zoning.gdb","special") %>%
...
```{r}
ap_shore_tab <- read_excel("data/PCAP - Shorezone Use Table.xlsx", sheet="Sheet1") %>%
...
```{r}
# gis
pas_gis <- st_read("zoning.gdb","zoning_local_plan_pas")
pas_special <- st_read("zoning.gdb","special_area_pas")
...
# spreadsheet
sheet1<-read_excel("data/PAS Matrix 10_16_02Expand_clean.xls", sheet="PAS 1 - 58_clean")
...
sheet2<-read_excel("data/PAS Matrix 10_16_02Expand_clean.xls", sheet="PAS 59 - 111_clean")
two <- sheet2 %>% gather(name, use, 4:118) %>%
separate(name ,c("code","name" ,"special_area","shorezone_tolerance"), sep="!") %>%
mutate(area_type="Plan Area Statement")
sheet3<-read_excel("data/PAS Matrix 10_16_02Expand_clean.xls", sheet="PAS 112 - 175_clean")
...
```{r}
#gis
cp_gis <- st_read("zoning.gdb","cp_gis")
cp_special <- st_read("zoning.gdb","cp_special_gis") %>%
...
#tabular
cp_sheet1<-read_excel("data/PAS Matrix 10_16_02Expand_clean.xls", sheet="CPs 001A-089B_clean")
cp_one <- cp_sheet1 %>% gather(name, use, 4:77) %>%
  separate(name ,c("code","name" ,"special_area","shorezone_tolerance"), sep="!")%>%
 mutate(area_type="Community Plan")
cp_sheet2<-read_excel("data/PAS Matrix 10_16_02Expand_clean.xls", sheet="CPs 091-125_clean")
...
zoning_gis <- rbind(gis_pas, st_transform(gis_area_plan, st_crs(gis_pas)),
  ...
  select(id, plan_name, area_type, use1, use2, use3, use) %>%
      mutate(use= case_when(use %in% c("A*","A","A**","(A)","A*****","A***","A1","A2","A3","A1/5") ~ "Allowed", 
             ...
             use2=str_trim(stri_trans_totitle(gsub("^.*\\.","", use2 )),side="left"),
             use2= case_when(use2 =="Tourist Accomodation" ~ "Tourist Accommodation",
                            use2 =="Watershed Improvement" ~ "Watershed Improvements",
                            use2 =="Os" ~ "Open Space",
                            use2 =="Wholesale / Storage" ~ "Wholesale/Storage",
                            TRUE ~ as.character(use2)))
...
write.csv(zoning_tab, file="zoning_tab.csv")

geojson_write(zoning_gis, file="zoning_gis.geojson")
